...
<div class="mx-auto flex max-w-md flex-col justify-center py-6 sm:py-14">
    ...
    <div data-reveal class="text-center">
        ...
        <h1 class="mt-5 font-judul text-3xl font-bold tracking-tight text-dasar-900">Selamat datang kembali</h1>
        <p class="mt-2 text-sm text-dasar-600">Lanjutkan mencari peluang yang cocok untukmu.</p>
    </div>
    ...
    <x-kartu datar data-reveal data-reveal-jeda="120" class="mt-8">
        <form method="POST" action="{{ route('masuk.proses') }}" class="space-y-5">
            ...
            <label class="flex cursor-pointer items-center gap-2.5 text-sm text-dasar-700">
                {{-- accent-color di CSS dasar yang mewarnai centangnya. Kelas
                     seperti text-utama-600 tidak berpengaruh pada kotak
                     centang bawaan. --}}
    ...
    <p data-reveal data-reveal-jeda="200" class="mt-6 text-center text-sm text-dasar-600">
        Belum punya akun?
        <a href="{{ route('daftar') }}"
           class="font-semibold text-utama-600 underline-offset-2 hover:underline">Daftar di sini</a>
    </p>
