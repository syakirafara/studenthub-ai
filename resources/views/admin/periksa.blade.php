...
@php
    ...
    $kelasIsian = 'mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-utama-600 focus:outline-none focus:ring-1 focus:ring-utama-600';
...
<a href="{{ route('admin.dasbor') }}" class="text-sm text-slate-500 hover:text-utama-600">
    &larr; Kembali ke antrean
</a>

<h1 class="mt-3 text-2xl font-semibold tracking-tight">Verifikasi peluang</h1>
...
@if ($duplikat->isNotEmpty())
    <div role="alert" class="mt-6 rounded-md border border-waspada-200 bg-waspada-50 px-4 py-3">
        <p class="text-sm font-medium text-waspada-800">
            Mirip dengan {{ $duplikat->count() }} peluang yang sudah ada
        </p>
        <ul class="mt-2 space-y-1 text-sm text-waspada-700">
        ...
        <p class="mt-2 text-xs text-waspada-700">
...
<div class="mt-6 grid gap-6 lg:grid-cols-5">
        ...
        <x-kartu class="sticky top-6 p-3">
            @if ($peluang->poster_path)
                <img src="{{ Storage::url($peluang->poster_path) }}"
                     alt="Poster {{ $peluang->judul }}" class="w-full rounded-md">
            @else
                <p class="p-6 text-center text-sm text-slate-500">Tidak ada poster.</p>
            @endif
    ...
    <div class="lg:col-span-3">
        <form method="POST" action="{{ route('admin.setujui', $peluang) }}" class="space-y-5">
            ...
            <div>
                <label for="judul" class="block text-sm font-medium text-slate-700">
                    ...
                    @if ($berubah('judul', old('judul', $peluang->judul)))
                        <span class="ml-1 text-xs font-normal text-waspada-700">dikoreksi</span>
                    @endif
                </label>
            ...
            <div>
                <label for="penyelenggara" class="block text-sm font-medium text-slate-700">Penyelenggara</label>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    <label for="tingkat" class="block text-sm font-medium text-slate-700">Tingkat</label>
                ...
                <div>
                    <label for="deadline" class="block text-sm font-medium text-slate-700">
                        Batas waktu
                        @if (! $peluang->deadline)
                            <span class="ml-1 text-xs font-normal text-waspada-700">tidak terbaca AI</span>
                        @endif
                    </label>
                ...
                <div>
                    <label for="biaya" class="block text-sm font-medium text-slate-700">Biaya</label>
                ...
                <div>
                    <label for="nominal_biaya" class="block text-sm font-medium text-slate-700">Nominal (Rp)</label>
                ...
                <div>
                    <label for="link" class="block text-sm font-medium text-slate-700">Tautan pendaftaran</label>
            ...
            <x-kartu class="bg-slate-50 p-4">
                <p class="text-sm font-medium text-slate-700">Syarat hasil bacaan AI</p>
                <pre class="mt-2 overflow-x-auto rounded bg-white p-3 text-xs text-slate-700">{{ json_encode($peluang->syarat, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) }}</pre>
        ...
        <form method="POST" action="{{ route('admin.tolak', $peluang) }}" class="mt-8 space-y-3">
            ...
            <label for="catatan_admin" class="block text-sm font-medium text-slate-700">
                Alasan penolakan
            </label>
