...
@php
    ...
    $kelasIsian = 'mt-1 w-full rounded-lg border border-white/12 px-3 py-2 text-sm focus:border-utama-400 focus:outline-none focus:ring-1 focus:ring-utama-500/30';
...
<a href="{{ route('admin.dasbor') }}" class="text-sm text-slate-400 hover:text-utama-300">
    &larr; Kembali ke antrean
</a>

<h1 class="mt-3 font-judul text-3xl font-bold tracking-tight text-white">Verifikasi peluang</h1>
...
<div class="mt-6 grid gap-6 lg:grid-cols-5">
    ...
    <div class="lg:col-span-2">
        <x-kartu class="sticky top-6 p-3">
            ...
            @else
                <p class="p-6 text-center text-sm text-slate-400">Tidak ada poster.</p>
            @endif
    ...
    <div class="lg:col-span-3">
        <form method="POST" action="{{ route('admin.setujui', $peluang) }}" class="space-y-5">
            ...
            <div>
                <label for="judul" class="block text-sm font-medium text-slate-200">
                    Judul
                    @if ($berubah('judul', old('judul', $peluang->judul)))
                        <span class="ml-1 text-xs font-normal text-waspada-300">dikoreksi</span>
                    @endif
                </label>
                <input id="judul" name="judul" type="text" required
                       value="{{ old('judul', $peluang->judul) }}" class="{{ $kelasIsian }}">
                @error('judul') <p class="mt-1 text-sm text-bahaya-300">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="penyelenggara" class="block text-sm font-medium text-slate-200">Penyelenggara</label>
                <input id="penyelenggara" name="penyelenggara" type="text"
                       value="{{ old('penyelenggara', $peluang->penyelenggara) }}" class="{{ $kelasIsian }}">
                @error('penyelenggara') <p class="mt-1 text-sm text-bahaya-300">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    <label for="tingkat" class="block text-sm font-medium text-slate-200">Tingkat</label>
                ...
                <div>
                    <label for="deadline" class="block text-sm font-medium text-slate-200">
                        Batas waktu
                        @if (! $peluang->deadline)
                            <span class="ml-1 text-xs font-normal text-waspada-300">tidak terbaca AI</span>
                        @endif
                    </label>
                    <input id="deadline" name="deadline" type="date"
                           value="{{ old('deadline', $peluang->deadline?->toDateString()) }}" class="{{ $kelasIsian }}">
                    @error('deadline') <p class="mt-1 text-sm text-bahaya-300">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="biaya" class="block text-sm font-medium text-slate-200">Biaya</label>
                ...
                <div>
                    <label for="nominal_biaya" class="block text-sm font-medium text-slate-200">Nominal (Rp)</label>
                    ...
                    @error('nominal_biaya') <p class="mt-1 text-sm text-bahaya-300">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="link" class="block text-sm font-medium text-slate-200">Tautan pendaftaran</label>
                    <input id="link" name="link" type="url" placeholder="https://..."
                           value="{{ old('link', $peluang->link) }}" class="{{ $kelasIsian }}">
                    @error('link') <p class="mt-1 text-sm text-bahaya-300">{{ $message }}</p> @enderror
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-3 border-t border-white/8 pt-5">
        ...
        <form method="POST" action="{{ route('admin.tolak', $peluang) }}" class="mt-8 space-y-3">
            ...
            <label for="catatan_admin" class="block text-sm font-medium text-slate-200">
                Alasan penolakan
            </label>
            ...
            @error('catatan_admin') <p class="text-sm text-bahaya-300">{{ $message }}</p> @enderror
