...
<p class="mt-1 text-sm text-slate-500">Masuk sebagai {{ auth()->user()->name }}</p>
    ...
    @foreach ([
        ...
        <x-kartu>
            ...
            <p class="mt-1 text-3xl font-semibold
                      {{ $warna === 'waspada' ? 'text-waspada-700' : ($warna === 'utama' ? 'text-utama-700' : 'text-slate-500') }}">
                {{ $angka }}
            </p>
        </x-kartu>
    @endforeach
...
{{-- Angka pemakaian AI --}}
<h2 class="mt-10 text-lg font-semibold">Pemakaian AI</h2>
...
<div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
    @foreach ([
        ...
        <x-kartu>
            ...
            @if ($catatan)
                <p class="mt-0.5 text-xs text-slate-500">{{ $catatan }}</p>
            @endif
        </x-kartu>
    @endforeach
</div>
...
<x-kartu class="mt-4">
    <div class="flex flex-wrap items-baseline justify-between gap-3">
        <div>
            <p class="text-xs uppercase tracking-wide text-slate-400">Akurasi pembacaan poster</p>
            @if ($ai['akurasi_persen'] !== null)
                <p class="mt-1 text-2xl font-semibold text-slate-800">{{ $ai['akurasi_persen'] }}%</p>
        ...
        <p class="text-xs text-slate-400">{{ number_format($ai['token_total'], 0, ',', '.') }} token terpakai</p>
    </div>
</x-kartu>

{{-- Antrean verifikasi --}}
<h2 class="mt-10 text-lg font-semibold">Antrean verifikasi</h2>

@if ($antrean->isEmpty())

    <x-kartu class="mt-4 border-dashed p-10 text-center">
        <p class="text-sm font-medium text-slate-700">Antrean kosong</p>
        <p class="mt-1 text-sm text-slate-500">Semua unggahan sudah diverifikasi.</p>
    </x-kartu>

@else

    <p class="mt-1 text-sm text-slate-500">Diurutkan dari yang paling lama menunggu.</p>
        ...
        @foreach ($antrean as $item)
            <x-kartu class="flex flex-wrap items-center justify-between gap-4">
                <div class="min-w-0">
                    ...
                    <p class="mt-1 font-medium text-slate-900">{{ $item->judul }}</p>
